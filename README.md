<div align="center">

# Hi, I'm Morty 👋

### Sysadmin by day · Streamer & Tool-Builder by night

*IT Administrator from Germany who keeps running into problems that need a script.*
*Some of those scripts end up here.*

[![Twitch Status](https://img.shields.io/twitch/status/mortys_welt?style=for-the-badge&logo=twitch&logoColor=white&label=mortys_welt&labelColor=9146FF&color=53FC18)](https://twitch.tv/mortys_welt)
[![YouTube](https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://youtube.com/@mortys_welt)
[![Website](https://img.shields.io/badge/mortys--welt.com-2F2F2F?style=for-the-badge&logo=googlechrome&logoColor=white)](https://mortys-welt.com)
<!-- TODO: replace YouTube URL with your actual channel handle if different from @mortys_welt -->

</div>

---

## 🧑‍💻 About Me

I run the IT infrastructure for a German manufacturing company by day - Active Directory, Exchange, Veeam, the usual sysadmin stack - and stream / create content on the side as a registered sole proprietor.

I'm **not** a professional developer. I keep finding gaps where the "right tool" either doesn't exist, has a terrible UI, or costs €4k/year per seat. So I build my own. The repos here are a mix of:

- 🛠️ **Sysadmin automation** - PowerShell, mostly. AD, Exchange, certificates, reporting, the kind of stuff that should already exist but somehow doesn't.
- 🎮 **Streaming & content tools** - OBS pipelines, Twitch/Discord bots, overlay widgets, post-stream automation.
- 🏠 **Homelab tinkering** - Proxmox, TrueNAS, self-hosted everything I can get my hands on.
- 📚 **Learning projects** - Currently working my way through C# / .NET 10 properly.

If something here is useful to you - great. If you spot a way to do it better - please tell me, that's why it's public.


## 🧰 Tech Stack

**Daily drivers**

![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat-square&logo=dotnet&logoColor=white)

**Infrastructure & Ops**

![Windows Server](https://img.shields.io/badge/Windows%20Server-0078D6?style=flat-square&logo=windows&logoColor=white)
![Active Directory](https://img.shields.io/badge/Active%20Directory-0078D4?style=flat-square&logo=microsoft&logoColor=white)
![Exchange](https://img.shields.io/badge/Exchange-0078D4?style=flat-square&logo=microsoftexchange&logoColor=white)
![Proxmox](https://img.shields.io/badge/Proxmox-E57000?style=flat-square&logo=proxmox&logoColor=white)
![TrueNAS](https://img.shields.io/badge/TrueNAS-0095D5?style=flat-square&logo=truenas&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)

**Streaming & Content**

![OBS Studio](https://img.shields.io/badge/OBS%20Studio-302E31?style=flat-square&logo=obsstudio&logoColor=white)
![Twitch](https://img.shields.io/badge/Twitch-9146FF?style=flat-square&logo=twitch&logoColor=white)
![Discord](https://img.shields.io/badge/Discord-5865F2?style=flat-square&logo=discord&logoColor=white)
![Streamer.bot](https://img.shields.io/badge/Streamer.bot-2F2F2F?style=flat-square&logo=streamlit&logoColor=white)

---

## ⭐ Featured Project

### 🎬 [MortysDLP](https://github.com/MortysTerminal/MortysDLP)

A user-friendly Windows app for downloading videos and audio from various platforms. Built in C# because the existing solutions either had clunky UIs or required command-line knowledge most people don't have.

> *"I needed something I could hand to a non-technical friend and have them actually use it."*

---
